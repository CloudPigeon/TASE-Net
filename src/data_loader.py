...
bert_tokenizer = BertTokenizer.from_pretrained('/home/u2023111000/MSA/src/premodels/bert-base-uncased', do_lower_case=True)
...
def get_loader(hp, config, shuffle=True):
    ...
    config.tva_dim = dataset.tva_dim#文本、视频、音频的隐藏维度(768,35,74)
    ...
    def collate_fn(batch):
        ...
        for sample in batch:
            if len(sample[0]) > 4: # unaligned case
                v_lens.append(torch.IntTensor([sample[0][4]]))#sample[0]的4和5维度分别代表视频和音频的长度
                a_lens.append(torch.IntTensor([sample[0][5]]))
            else:   # aligned cases
                v_lens.append(torch.IntTensor([len(sample[0][3])]))#对齐后长度一致
                a_lens.append(torch.IntTensor([len(sample[0][3])]))
            # labels.append(torch.from_numpy(sample[1]))#sample[1]代表标签
            r_labels.append(torch.from_numpy(np.array(sample[1])))#sample[1]代表标签
            c_labels.append(torch.from_numpy(np.array(sample[2])))#sample[1]代表标签
            ids.append(sample[2])
        vlens = torch.cat(v_lens)
        alens = torch.cat(a_lens)
        # labels = torch.cat(labels, dim=0)
        r_labels = torch.stack(r_labels).unsqueeze(1)
        c_labels = torch.stack(c_labels).long()
        ...
        def pad_sequence(sequences, target_len=-1, batch_first=True, padding_value=0.0):
            ...
            if batch_first:
                ...
                out_dims = (max_len, len(sequences)) + trailing_dims#根据参数 batch_first 确定输出张量的维度。如果 batch_first 为True，维度顺序为(batch, seq, trailing_dims)；否则为(seq, batch, trailing_dims)。
        ...
        return visual, vlens, acoustic, alens, r_labels,c_labels, lengths, bert_sentences, bert_sentence_att_mask, ids
