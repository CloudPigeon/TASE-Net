    ...
    def __init__(self, config):
        ...
        try:
            ...
            if True:
                ...
                r_labels = np.concatenate((train_split_noalign['regression_labels'],dev_split_noalign['regression_labels'], test_split_noalign['regression_labels']),axis=0)
                c_labels = np.concatenate((train_split_noalign['classification_labels'],dev_split_noalign['classification_labels'], test_split_noalign['classification_labels']),axis=0)
            ...
            for i, idd in enumerate(all_id_list):
                ...
                _rlabel = r_labels[i].astype(np.float32)
                _clabel = c_labels[i].astype(np.int32)
                _visual = v[i]
                _acoustic = a[i]
                _vlen = vlens[i]
                _alen = alens[i]
                ...
                if i < dev_start:
                    train.append(((words, visual, acoustic, actual_words, _vlen, _alen), _rlabel, _clabel , idd))
                elif i >= dev_start and i < test_start:
                    dev.append(((words, visual, acoustic, actual_words, _vlen, _alen), _rlabel,_clabel, idd))
                elif i >= test_start:
                    test.append(((words, visual, acoustic, actual_words, _vlen, _alen), _rlabel,_clabel, idd))
                else:
...
class MOSEI:
    def __init__(self, config):
        ...
        try:
            ...
            # load pickle file for unaligned acoustic and visual source
            pickle_filename = '../datasets/MOSEI/unaligned_50.pkl'
            ...
            r_labels = np.concatenate((train_split_noalign['regression_labels'],dev_split_noalign['regression_labels'], test_split_noalign['regression_labels']),axis=0)
            c_labels = np.concatenate((train_split_noalign['classification_labels'],
                                       dev_split_noalign['classification_labels'],
                                       test_split_noalign['classification_labels']), axis=0)
            L_V = v.shape[1]
            L_A = a.shape[1]
